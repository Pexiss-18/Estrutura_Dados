...
def partition(arr, left, right):
    ...
    for j in range(left, right):# Percorre todos os elementos do subarray.
        if arr[j] <= pivot:# Se o elemento atual for menor ou igual ao pivô...
            i += 1# Avança o ponteiro de troca.
            arr[j], arr[i] = arr[i], arr[j]# Troca o elemento atual com o elemento na posição de troca.
    arr[i], arr[right] = arr[right], arr[i]# Após o loop, coloca o pivô na posição correta (entre os menores e os maiores). 
